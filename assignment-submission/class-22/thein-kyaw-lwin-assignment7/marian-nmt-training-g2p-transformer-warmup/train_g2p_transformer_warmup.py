"""
train_g2p_transformer_warmup.py
================================
Experiment 1: LR Warmup ONLY
  --lr-warmup 4000          ← CHANGED  (baseline was 0)
  --transformer-dropout 0.3 ← unchanged
...
MODEL_DIR    = os.path.join(WORKING_DIR, "model.transformer.myph.warmup")
...
# ---------------------------------------------------------------------------
LR_WARMUP           = 4000   # ← CHANGED from 0
TRANSFORMER_DROPOUT = 0.3    # ← unchanged (baseline)

print(f"\n[config] Experiment : WARMUP ONLY")
print(f"[config] lr-warmup            : {LR_WARMUP}  (baseline=0)")
print(f"[config] transformer-dropout  : {TRANSFORMER_DROPOUT}  (baseline=0.3, unchanged)")
...
hyp_file = os.path.join(WORKING_DIR, f"transformer.{SRC}{TGT}.warmup.hyp.txt")
...
print(f"\n[DONE] G2P Transformer training complete — Experiment: WARMUP ONLY")
