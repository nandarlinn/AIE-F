...
Reference: Transformer-NMT-marian-ph2gp.ipynb (Ye Kyaw Thu, 26 May 2026)
"""
...
MODEL_DIR    = os.path.join(WORKING_DIR, "model.transformer.myph")
...
section("Step 3: Train Transformer G2P model  (my → ph)  on 2x Tesla T4")
...
marian_args = " ".join([
    ...
    f"--transformer-dropout 0.3 --label-smoothing 0.1",
    f"--learn-rate 0.0003 --lr-warmup 0 --lr-decay-inv-sqrt 16000 --lr-report",
    f"--clip-norm 5",
    f"--tied-embeddings",
...
# NOTE: 'time' is a bash builtin — use Python's time module for elapsed timing
...
hyp_file = os.path.join(WORKING_DIR, f"transformer.{SRC}{TGT}.hyp.txt")
...
print(f"\n[DONE] G2P Transformer training complete.")
