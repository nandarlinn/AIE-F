"""
train_g2p_s2s_lr_schedule_smoothing.py
=======================================
Experiment 2: LR Schedule + Label Smoothing + Decoding Fixes COMBINED
  ...
  --label-smoothing 0.1         ← CHANGED  (baseline was 0 / disabled)
  --normalize 0.6               ← CHANGED  (baseline was 0 / disabled)
  --beam-size 6                 ← CHANGED  (baseline default was 12)
  --clip-norm 5                 ← CHANGED  (baseline default was 1)
...
MODEL_DIR    = os.path.join(WORKING_DIR, "model.s2s.myph.lr_schedule_smoothing")
...
LABEL_SMOOTHING   = 0.1      # ← CHANGED from 0 (disabled in baseline)
NORMALIZE         = 0.6      # ← CHANGED from 0 (disabled in baseline)
BEAM_SIZE         = 6        # ← CHANGED from default 12
CLIP_NORM         = 5        # ← CHANGED from default 1

print(f"\n[config] Experiment : LR SCHEDULE + LABEL SMOOTHING + DECODING FIXES")
...
print(f"[config] label-smoothing      : {LABEL_SMOOTHING}  (baseline=0, disabled)")
print(f"[config] normalize            : {NORMALIZE}  (baseline=0, disabled)")
print(f"[config] beam-size            : {BEAM_SIZE}  (baseline default=12)")
print(f"[config] clip-norm            : {CLIP_NORM}  (baseline default=1)")
...
section(f"Step 3: Train s2s G2P model  (my → ph)  [LR={LEARN_RATE}, inv-sqrt={LR_DECAY_INV_SQRT}, smooth={LABEL_SMOOTHING}]")
...
marian_args = " ".join([
    ...
    f"--lr-report",                     # ← CHANGED
    f"--label-smoothing {LABEL_SMOOTHING}",  # ← CHANGED
    f"--normalize {NORMALIZE}",         # ← CHANGED (beam decoding length penalty)
    f"--beam-size {BEAM_SIZE}",         # ← CHANGED
    f"--clip-norm {CLIP_NORM}",         # ← CHANGED
...
_t0 = _time.time()
run(f"{MARIAN} -c {MODEL_DIR}/config.yml 2>&1 | tee {MODEL_DIR}/train.s2s.myph.lr_schedule_smoothing.log")
...
hyp_file = os.path.join(WORKING_DIR, f"s2s.{SRC}-{TGT}.lr_schedule_smoothing.hyp.txt")
...
print(f"\n[DONE] G2P s2s training complete — Experiment: LR SCHEDULE + LABEL SMOOTHING + DECODING FIXES")
