...
def get_line_save_counts(user_name):
    """
    Returns a dict mapping 0-based line index (as string) → number of saved files.
    e.g. {"0": 3, "1": 2, "1374": 1}

    Files are named like  <1-based-line>-<sample>.txt  →  "1371-2.txt"
    """
...
def user_progress(user_name):
    """Legacy: number of distinct lines that have at least one sample."""
    path = get_user_path(user_name)
    if not os.path.isdir(path):
        return 0
    written = set()
