...
my @configs = `find /mnt/data/personal_projects/NandarLinn-SMT-Assignment/pbsmt-normalize/*/ -name "config.baseline*" | sort`;
...
#	chomp $b_config;
	...
	`/home/elio/tool/mosesbin/ubuntu-17.04/moses/scripts/ems/experiment.perl -config $config -exec -no-graph 2>&1 | tee -a run1.log`;
